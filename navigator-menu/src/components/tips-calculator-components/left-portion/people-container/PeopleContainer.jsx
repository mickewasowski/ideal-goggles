...
function PeopleContainer(){
    const {people, tipsCalc} = useContext(TipsCalculatorContext); 
    ...
    return(
        <Container>
            ...
            <ErrorLabel id="error-label" style={error ? {display: 'inline'} : {display: 'none'}}>Can't be zero</ErrorLabel>
            <InputWrapper className={error ? errorDiv : ''}>
                ...
                <Input type="number" id="people-count" onChange={handleChange} />
            </InputWrapper>
        </Container>
    )
}
